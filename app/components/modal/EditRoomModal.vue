...
const onCountryChange = async (): Promise<void> => {
  ...
  try {
    counties.value = await $fetch<County[]>(`${config.public.apiBase}/api/Counties/${form.CountryId}`)
    cities.value   = []
    form.CountyId  = ''
    form.CityId    = ''
  }
...
const onCountyChange = async (): Promise<void> => {
  ...
  try {
    cities.value = await $fetch<City[]>(`${config.public.apiBase}/api/Cities/${form.CountyId}`)
    form.CityId  = ''
  }
  catch (err) {
...
onMounted(async () => {
  try {
    const [countryRes, tagRes, countyRes] = await Promise.all([
      ...
      form.CountryId ? $fetch<County[]>(`${config.public.apiBase}/api/Counties/${form.CountryId}`) : Promise.resolve([] as County[]),
    ])
    ...
    <div class="modale" :class="{ opened: visible }">
      <div class="modal-dialog">
          ...
          <div class="modal-body-l">
            ...
            <div class="room-photo-group group-flex mb">
              ...
              <div class="room-img-wrapper" @click="triggerFileInput">
                <img v-if="form.Picture" :src="form.Picture" class="room-img" alt="招牌圖片" />
            ...
            <div class="access-review-limit group-flex mb">
              <label>最低評價要求</label>
              <select>
        ...
        <div class="modal-footer group-flex">
          ...
          <button class="btn-action btn-action--primary" :disabled="isSubmitting" @click="saveRoom">
            {{ isSubmitting ? '儲存中…' : '確定' }}
          </button>
        </div>
      </div>
...
.ribon001 { position: absolute; width: 90px; top: -40px; left: 0; }
.creatroom-title {
  ...
  .star001 { margin-left: 8px; width: 30px; }
}
.modal-body {
  display: flex;
  justify-content: center;
  ...
  height: 400px;
  overflow-y: auto;
  label { color: $color-brown; margin-right: 15px; font-size: 20px; font-family: myfont, japanese-font, serif; }
  input[type='text'] { height: 28px; border: none; border-bottom: 2px solid lighten($color-brown, 10%); font-size: 20px; padding-left: 8px; font-family: myfont, japanese-font, serif; color: $color-brown; }
  select { height: 28px; font-family: japanese-font, myfont, serif; color: $color-brown; border: none; margin-right: 8px; font-size: 18px; border-bottom: 2px solid $color-brown; }
}
.modal-body-l { margin-right: 25px; }
.modal-body-r {
  margin-left: 25px;
  textarea { resize: none; white-space: pre-wrap; padding: 10px; font-family: myfont, japanese-font, serif; color: $color-brown; font-size: 18px; &:focus { outline: none; } }
  .rule-title { margin-bottom: 15px; }
}
.group-flex { display: flex; align-items: center; }
.mb { margin-bottom: 14px; }
.room-img-wrapper { cursor: pointer; }
.room-img { width: 200px; height: 100px; border-radius: 5px; display: flex; align-items: center; justify-content: center; object-fit: cover; &--empty { background-image: url('/image/addPicture.png'); background-size: cover; span { color: $color-brown; font-size: 16px; } } }
.room-name { width: 70%; }
.location-selects { display: flex; flex-wrap: wrap; gap: 4px; }
.hidden-file-input { display: none; }
.r18-request { display: flex; align-items: center; span { margin-left: 10px; font-size: 20px; font-family: myfont, japanese-font, serif; color: $color-brown; } }
.modal-footer { margin: 10px 10px 10px 0; display: flex; justify-content: flex-end; align-items: center; }
.btn-action {
  height: 38px; margin-left: 10px; width: 80px;
  font-family: myfont, japanese-font, serif; font-size: 20px; color: #fff;
  background-color: $color-brown; border: none; border-radius: 5px; cursor: pointer;
  &:hover { background-color: lighten($color-brown, 10%); }
  &:disabled { opacity: 0.6; cursor: not-allowed; }
  &--primary { background-color: darken($color-header, 10%); &:hover { background-color: darken($color-header, 15%); } }
}
</style>
