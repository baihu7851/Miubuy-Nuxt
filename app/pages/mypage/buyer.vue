...
const fetchOrders = async () => {
  try {
    orders.value = await $fetch<BuyerOrder[]>(
      `${config.public.apiBase}/api/BuyerRatings`,
      {
        headers: { Authorization: `Bearer ${token.value}` },
      },
    );
  } catch (err) {
    console.error('取得買家訂單失敗', err);
  }
};
...
<template>
  <div class="mypage-order-content">
    ...
    <div class="deco03">
      <img src="/image/buyer.png" alt="" class="buyer-icon" />
    </div>
          ...
          <div class="title-group">
            ...
            <h3 class="review">
              ...
              <p class="order-review">
                {{ order.BuyerStar }}<br /><span>{{ order.BuyerReviews }}</span>
              </p>
            </h3>
          </div>
        </div>
