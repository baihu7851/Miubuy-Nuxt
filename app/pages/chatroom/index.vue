...
onMounted(async () => {
  ...
  selectedCountyId.value =
    Number(sessionStorage.getItem('prefecture') ?? '') || '';
  selectedCityId.value = Number(sessionStorage.getItem('city') ?? '') || '';

  await Promise.all([fetchRooms(), fetchTags(), fetchCounties()]);
});
...
const fetchRooms = async (tagId?: number): Promise<void> => {
  try {
    rooms.value = await $fetch<Room[]>(
      `${config.public.apiBase}/api/SelectRooms`,
      {
        method: 'POST',
        body: {
...
const fetchCounties = async (): Promise<void> => {
  ...
  try {
    counties.value = await $fetch<County[]>(
      `${config.public.apiBase}/api/Counties/${countryId.value}`,
    );
  } catch (err) {
...
const onCountyChange = async (): Promise<void> => {
  ...
  try {
    cities.value = await $fetch<City[]>(
      `${config.public.apiBase}/api/Cities/${selectedCountyId.value}`,
    );
  } catch (err) {
    ...
    <div class="max-width">
      ...
      <div class="select-group">
        ...
        >
          <option value="">
            {{
              counties.find((c) => c.Id === selectedCountyId)?.Name ?? '縣市'
            }}
          </option>
          <option
        ...
        <select v-model="selectedCityId" class="select-area">
          <option value="">
            {{ cities.find((c) => c.Id === selectedCityId)?.Name ?? '城市' }}
          </option>
      ...
      <div class="chatroomlist-container">
        ...
        <!--
          TODO ☆ 關鍵字搜尋功能
          Vue2 原版搜尋列同樣沒有綁定（UI-only）
          目前後端 API 未提供關鍵字搜尋參數
          確認後補上 v-model + @click 送出邏輯，並加上無結果提示
        -->
        <!-- 搜尋列（UI 保留） -->
            ...
            <li v-for="room in rooms" :key="room.Id" class="chatroom">
              ...
              <div class="user-group">
                <div
                  class="profile-img"
                  :style="{
                    backgroundImage: `url(${room.Seller[0]?.Picture})`,
                  }"
                />
                <div class="user">
...
.map-select-jp {
  ...
  position: absolute;
  top: -280px;
  background-image: url('/image/cat_map_img.png');
  background-size: cover;
  width: 140px;
  height: 110px;
  margin-left: 50px;
...
.map-select-tw {
  ...
  top: -280px;
  right: 50px;
  background-image: url('/image/cat_map_img_tw.png');
  background-size: cover;
  width: 140px;
  height: 110px;
  transform: translateY(-10px);
...
.room-info {
  ...
  .hr {
    width: 100%;
    background-color: lighten($color-brown, 30%);
    height: 1px;
    margin-bottom: 15px;
  }
}
...
.room-enter-btn {
  background-image: url('/image/enter_btn.png');
  background-size: cover;
  position: absolute;
  width: 100px;
  height: 60px;
...
.page-1st {
  @include page-btn(66px, 0px, -5deg);
}
.page-2nd {
  @include page-btn(95px, 1px, 5deg);
}
.page-3rd {
  @include page-btn(127px, -2px, -3deg);
}
.page-4th {
  @include page-btn(157px, 8px, 5deg);
}
